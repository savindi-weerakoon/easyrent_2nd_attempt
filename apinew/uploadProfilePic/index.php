  ...
  function profileUpload()
  {
    ...
    if ($image['size'] > 0 && $image['error'] == 0) { //check whether image is empty 
      ...
      if (!in_array($ext, $allowed)) {
        ...
        $image_upload_dir = './../uploads/profile/' . $image_name;
        ...
        if (move_uploaded_file($temp_name, $image_upload_dir)) {
          ...
          $response = array(
            "status" => "error",
            "error" => true,
            "message" => "Error when uploading profile image"
          );
          http_response_code(403);
          return json_encode($response);
        }
      ...
      $response = array(
        "status" => "error",
        "error" => true,
        "message" => "Error when reading profile image"
      );
      http_response_code(403);
      return json_encode($response);
    }
...
echo $Uploadprofile->profileUpload();
